...
void moveVertical(int distInInches, int speed = 100, bool seeking = false) {
	...
	motor[frontRight] = motor[backRight] = BOUND(encoderDist * kp, -speed,
			speed);
	while (nMotorRunState[frontLeft] != runStateIdle
			|| nMotorRunState[frontRight] != runStateIdle) {
		...
		if (seeking && SensorValue[irSeeker] == 5) //if the robot is in "seeking" mode and it finds the beacon
				{
			motor[frontLeft] = motor[backLeft] = motor[frontRight] =
					motor[backRight] = 0;
			return;
		}
		...
		if (lError <= 0 || rError <= 0) {
			...
			motor[frontLeft] = motor[backLeft] = BOUND(lError * kp, -speed,
					speed);
			motor[frontRight] = motor[backRight] = BOUND(rError * kp, -speed,
					speed);
		}
	}
}
		...
		if (onArrayIndex % 2 == 0) {
			while (nNxtButtonPressed != 3) {
				if (nNxtButtonPressed == 1) {
					...
					nxtDisplayString(onArrayIndex, "Wait %d seconds",
