...
/* Janurary 8th, 2014 (EDIT [2/7/15]: It's 2015, not 2014...)
...
void moveStraight(int distInInches) {
	...
	while (abs(nMotorEncoder[liftLeftTop]) < abs(encoderDist)
			&& abs(nMotorEncoder[liftRightBottom]) < abs(encoderDist)) {
...
void selectStart() {
	...
	while (nNxtButtonPressed != 3) {
		if (nNxtButtonPressed == 1 || nNxtButtonPressed == 2) {
			if (platformStart == 0)
				...
				platformStart = 0;
			if (platformStart == 0)
				nxtDisplayString(1, "!Top");
			else
				nxtDisplayString(1, "!Bottom");
			wait1Msec(250);
		}
		wait1Msec(250);
	}
	...
	while (nNxtButtonPressed != 3) {
		if (nNxtButtonPressed == 1 || nNxtButtonPressed == 2) {
			get90Cm = !get90Cm;
			if (get90Cm == true)
				nxtDisplayString(4, "!Yes");
			else
				nxtDisplayString(1, "!No");
			wait1Msec(250);
		}
		wait1Msec(250);
	}
...
task main() {
	...
	nMotorEncoder[liftLeftTop] = 0;
	nMotorEncoder[liftRightBottom] = 0;
	while (abs(nMotorEncoder[liftLeftTop]) > -4500
			&& abs(nMotorEncoder[liftRightBottom]) < 4500) {
		if (nMotorEncoder[liftLeftTop] != -nMotorEncoder[liftRightBottom]) {
			error = nMotorEncoder[liftLeftTop] + nMotorEncoder[liftRightBottom];
		}
